USE storefrontdb;
#2.1 Insert values in respective table.
...
values(1, 'earphones', 1500.0, 5, true, 'C:\Users\Admin\MysqlWorkspace\earphones.jpg', '2018-01-01', 'jbl earphones'),
      ...
      (12, 'notpads', 50.0, 10, true, null, '2018-12-01', 'shree notepads'),
      (13, 'pens', 10.0, 20, true, 'C:\Users\Admin\MysqlWorkspace\pens.jpg', '2019-08-01', 'use and throw'),
      (14, 'marker', 20.0, 0, false, null, '2018-12-01', 'red glass marker')
	;
...
INSERT INTO product_categorised(productId, categoryId)
values(1,3),
      (2,3),
      (3,4),
      (4,4),
...
values(1, 1, '117', 'ranjeet', 'jaipur', 'rajasthan', 'INDIA', 302012),
      ...
      (7, 8, '420', 'narayn', 'jodhpur', 'rajasthan', 'INDIA', 602012),
...
values(102, 1, 1, 1, 'returned'),
      ...
      (253, 3, 4, 1, 'shipped'),
      (295, 13, 9, 1, 'shipped'),
      (310, 10, 7, 2, 'awaiting shipping'),
      (322, 5, 11, 1, 'order placed'),
      (324, 2, 2, 1, 'order placed'),
      (325, 5, 2, 1, 'awaiting shipping'),
...
#2.2 Display Id, Title, Category Title, Price of the products which are Active and recently added products should be at top.
SELECT p.id, p.name, p.price, c.name, p.added_on_date
FROM product p, product_categorised pc, category c
WHERE p.is_active = true AND p.id = pc.productId AND pc.categoryId = c.id
ORDER BY p.added_on_date DESC;

#2.3 Display the list of products which don't have any images.
SELECT DISTINCT p.id, p.name
FROM product p
WHERE p.product_image IS NULL;

#2.4 Display all Id, Title and Parent Category Title for all the Categories listed, sorted by Parent Category Title and then Category Title. 
...
#2.5 Display Id, Title, Parent Category Title of all the leaf Categories (categories which are not parent of any other category)
SELECT c1.id, c1.name, c1.root
FROM category c1
WHERE c1.id NOT IN (SELECT c2.root FROM category c2 WHERE c2.root IS NOT NULL)
;

#2.6 Display Product Title, Price & Description which falls into particular category Title
SELECT p.name, p.price, p.description, c.name
FROM product p, category c, product_categorised pc
WHERE p.id = pc.productId AND c.id = pc.categoryId ;
...
#2.8 Increase the Inventory of all the products by 100.
UPDATE product
